...
import { DEFAULT_TIPS, ProgressDisplay, type ProgressStep } from "./ProgressDisplay";
...
describe("ProgressDisplay", () => {
  ...
  it("renders progress percentage", () => {
    render(<ProgressDisplay currentStep={0} progress={25} steps={sampleSteps} />);
  ...
  it("shows time remaining", () => {
    render(
      <ProgressDisplay currentStep={0} progress={50} steps={sampleSteps} timeRemaining={7.2} />,
    );
    expect(screen.getByText("残り約 8 秒")).toBeInTheDocument();
  });

  it("renders all step labels", () => {
    render(<ProgressDisplay currentStep={1} progress={60} steps={sampleSteps} />);
  ...
  it("clamps time remaining to minimum of 1", () => {
    render(
      <ProgressDisplay currentStep={0} progress={99} steps={sampleSteps} timeRemaining={0.3} />,
    );
  ...
  it("rotates the creative tip every four seconds", () => {
    vi.useFakeTimers();
    render(<ProgressDisplay currentStep={0} progress={50} steps={sampleSteps} />);
    expect(screen.getByText(DEFAULT_TIPS[0])).toBeInTheDocument();

    act(() => {
      vi.advanceTimersByTime(4000);
    });
    expect(screen.getByText(DEFAULT_TIPS[1])).toBeInTheDocument();

    act(() => {
      vi.advanceTimersByTime(4000);
    });
    expect(screen.getByText(DEFAULT_TIPS[2])).toBeInTheDocument();
  });
});
